...
func tests(t *testing.T, r *RedisClient, hosts, password string, db int) {
	...
	t.Run("1 Get()", func(t *testing.T) {
		if r.client == nil {
			r.cluster.Del(ctx, key)
		} else {
			r.client.Del(ctx, key)
		}
		got, err := r.Get(ctx, key, defaultValue)
		require.NoError(t, err, "Get()")
		require.Equal(t, defaultValue, got, "Get()")

		if r.client == nil {
			r.cluster.Set(ctx, key, value, time.Duration(10)*time.Second)
			defer r.cluster.Del(ctx, key)
		} else {
			r.client.Set(ctx, key, value, time.Duration(10)*time.Second)
	...
	t.Run("3 Set with timer", func(t *testing.T) {
		var got string
		err := r.Set(ctx, key, value, 1)
		require.NoError(t, err, "Set()")
		if r.client == nil {
			defer r.cluster.Del(ctx, key)
			got, err = r.cluster.Get(ctx, key).Result()
		} else {
			defer r.client.Del(ctx, key)
		...
		if r.client == nil {
			_, err = r.cluster.Get(ctx, key).Result()
		} else {
	...
	t.Run("4 Keys()", func(t *testing.T) {
		...
		r.client.Set(ctx, key1, faker.Word(), time.Duration(10)*time.Second)
		defer r.client.Del(ctx, key1)
		r.client.Set(ctx, key2, faker.Word(), time.Duration(10)*time.Second)
		defer r.client.Del(ctx, key2)

		keys, err := r.Keys(ctx, fmt.Sprintf("%s:*", pattern))
	...
	t.Run("7 MultiSet()", func(t *testing.T) {
		...
		for _, set := range sets {
			var got string
			if r.client == nil {
				got, err = r.cluster.Get(ctx, set.Key).Result()
				defer r.cluster.Del(ctx, set.Key)
			} else {
				got, err = r.client.Get(ctx, set.Key).Result()
				defer r.client.Del(ctx, set.Key)
			}
			require.NoError(t, err, "redis.Get()")
	...
	t.Run("8 Streams", func(t *testing.T) {
		...
		err := r.XGroupCreateMkStream(ctx, stream, group, "$")
		require.NoError(t, err, "XGroupCreateMkStream()")

		defer r.XGroupDestroy(ctx, stream, group)
		...
		data, err := r.XReadGroup(ctx, &readArgs)
